...
class SqlDb{
  ...
  initialDb() async {
    ...
    Database tipsDb = await openDatabase(join(dbPath,'beauty_tips.db'), onCreate: _onCreate, version: 1, onUpgrade: _onUpgrade);
    return tipsDb;
  }
  ...
  _onUpgrade(Database db, int oldVersion, int newVersion) async{
    await db.execute(
      ...
      */
        '''
        INSERT INTO tips (content, typeID, title) VALUES ('Trim your nails regularly keeping them at a comfortable length. File them gently in one direction to avoid splitting or tearing.', 3, 'Keep Nails Trimmed');
        ''');
    print(" Done ");
  }
